...
export async function repositoryExists(sourceRepository) {
    const {owner, repo} = parseRepository(sourceRepository)

    if (owner === 'users') {
        if (repo === 'USER') return false; // unfortunately the user USER actually exists
...
export async function downloadGithubReleaseAsset({owner, repo, releaseTag, fileName}) {
    ...
    if (assetResponse.status === 200 || assetResponse.status === 304) {
        const assetAge = Math.floor((new Date() - new Date(assetResponse.headers.get('last-modified'))) / 1000)
        return {
